...
public class Trap extends ISA {
	...
	public static void trap(){
		if(CPU.getInstance().getControler().trap) {
			...
			CPU.cyclePlusOne();
			Halt.halt();
			CPU.getInstance().getMBR().setContent(Cache.getInstance().getdata(CPU.getInstance().getMAR().getContent()));
			CPU.cyclePlusOne();
			Halt.halt();
			...
			CPU.getInstance().getMBR().setContent(Cache.getInstance().getdata(CPU.alignment("0")));
			CPU.cyclePlusOne();
			Halt.halt();
